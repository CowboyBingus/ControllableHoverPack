...
Run `python -B scripts/build.py`. It runs the focused tests and creates `releases/Controllable-Hover-Pack-v1.zip`. In the multi-mod workspace, releases share the parent directory. Intermediate files stay in ignored `build/`. The builder never installs the mod or launches the game.
